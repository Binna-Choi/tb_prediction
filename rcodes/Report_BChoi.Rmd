---
title: "Analysis of Incidences of Tuberculosis (TB)"
author: "Bich Na Choi"
date: "2023-12-15"
output:
...
setwd("/Users/bchoi/Google Drive/LMU Homework Tasks/data")
...
```{r tb data processing, include=FALSE}
...
```{r figure1.a, warning = FALSE, fig.align="center", out.width="75%"}
...
```{r figure1.b, warning = FALSE, fig.align="center", out.width="75%"}
...
```{r fig2 data preprocessing, include=FALSE, fig.cap="Global trends in the TB incidence rate by WHO region in 2022"}
...
# 
```
```{r exercise2 figure, warning=FALSE, fig.width=12, fig.asp=0.518, out.width="100%", fig.align="default", echo=TRUE}
...
Figure 2.a shows the relationship between GDP per capita and TB incidence per 100 000. It shows that as GDP per capita increases, the incidence of TB decreases exponentially. Figure 2.b shows the negative linear relationship between GDP per capita and TB incidence, with a logarithmic scale. In conclusion, the wealth of a country is very closely related to the incidence of TB. To explore the relationship between GDP and TB incidence rates in the past, scatter plots are provided for the years 2000, 2005, 2010, 2015, 2020 and 2022. 
```{r figures, warning=FALSE, fig.align="center", out.width="85%",}
...
```{r exercise3 data preprocessing, include = FALSE}
...
The variables that are not available in 2022 have been dropped. In addition, variables with 50% missing values are deleted. Categorical variables are not considered as a predictor variables except WHO region variable (`g_whoregion`). The `g_whoregion` values were transformed one-hot encoded. 

Finally, the 201 countries and 169 predictor variables are considered in the log regression model. 


```{r exercise 3 figure3.1, fig.align="center", warning=FALSE, out.width="75%"}
...
The above figure shows the distribution of the new TB cases in 2022. The outcome variable is count data and the distribution is positive skewed. Hence, poisson canonical link function is used for developing a generalized linear model. 
...
```{r exercise 3 figure3.2, fig.align="center", warning=FALSE, out.width="75%"}
par(mfrow = c(2, 2))
plot(fit_tb)
```
